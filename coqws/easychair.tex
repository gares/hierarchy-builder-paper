...
\paragraph{A hierarchy with a diamond in 50 lines}
...
\begin{wrapfigure}[0]{r}{.35\textwidth}
  ...
  \vspace{-1.5em}
  %\caption{
    \center{\small{Figure 1: The hierarchy}}
   % }
  \label{fig:hierarchy}
\end{wrapfigure}
\
...
(* We need no additional mixin to declare the structure of rings. *)
...
of the green arrows (Figure 1) which are alternative interfaces
called factories. They provide specialized interfaces to build structures for others.
...
can omit the one for the commutativity of the addition, since it is implied
