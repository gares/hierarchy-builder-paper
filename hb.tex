...
\end{coqcode}
  Creates a module \coq{M} with a record which is exported as
  \coq{M}, and which depends on the \mixins{}
...
#\hbend{}#.
\end{coqcode}
  Creates a module \coq{F} with a record which is exported as
  \coq{F}, and which depends on the mixins
  \(\requires{}(\coq{M}) =
  \provides{}^\star\left(\enum{f_1}{f_n}\right)\) and registers this
  new record as a factory.  It also exports an
  \phantterm{} \coq{F.Build.} We then use the \factoryinstances{}
...
A \class{} \coq{c} \(\in \C{}\) is distinguished a Coq record with one parameter
...
The full code of this V4 is available in Appendix~\ref{sec:full-v4-code}.
...
(* Builder arrow from top right to bottom left *)
