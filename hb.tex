...
that equips \coq{A} with a list of \factories{}.
...
the \coq{Ring_of_AbelianGroup.Build} \phantterm{} that requires \coq{A} to be
an \coq{AbelianGroup}.
...
\begin{definition}[$\requires{}$, factory requirements.]
  ...
  as the set of all \mixins{} that occur as parameters of \(f\), i.e.
  \(\dep{}(f) = \enum{m_1}{m_n}\).
\end{definition}
...
\begin{definition}[$\provides{}$]
  \( \provides{}(f) = \{m | \from{}(f,m) \textrm{ is defined}\}
...
different from regular \factories{}, since they are a special cases.
...
\end{coqcode}
  Declares the record \coq{M} inside an homonymous module \coq{M}.
  The record \coq{M.M} depends on the \mixins{}
  ...
  \coq{M} standing for \coq{M.M}, so that the module name can be used to
  denote the homonymous record it contains.
\end{command}
...
#\hbend{}#.
\end{coqcode}
  Declares the record \coq{F} inside an homonymous module \coq{F}.
  The record \coq{F.F} depends on the mixins
  \(\requires{}(\coq{M}) =
  \provides{}^\star\left(\enum{f_1}{f_n}\right)\) and is registered
  as a factory.  It exports an
  \phantterm{} \coq{F.Build.} and a notation
  \coq{F} standing for \coq{F.F}, so that the module name can be used to
  denote the homonymous record it contains.
...
can obtain \mixinbuilders{} out of each \(b_i\) by function composition.
...
\coq{Monoid} consists just one \mixin{}. In contrast the class record
...
HB.builders Context (A : Type) (f : Ring_of_Monoid A).
  ...
  Definition to_AbelianGroup_of_Monoid := AbelianGroup_of_Monoid.Build A opp_f addrC addNr_f.
  HB.instance A to_AbelianGroup_of_Monoid.

  (* Builder to the top right factory, which is compiled to the bottom left mixin. *)
  Definition to_Ring_of_AbelianGroup := Ring_of_AbelianGroup.Build A one_f mul_f
